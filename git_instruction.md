![Git logo](Логотип-removebg-preview.png)
# Работа с Git
...
``` 
` После решения конфликта, нужно сделать коммит слиятния `
## 3. Удаление веток
...
Для того что бы добавить сокращенную команду нужно: 
использовать команду `git config --global alias.<сокращение> "команда" ` *пример*
```
 git config --global alias.st "status" ,это позволит использовать сокращение st для команды status
``` 

# Раздел 5. Работа с удаленными репозиториями
 * Создать аккаунт на [сайте](https://github.com/) https://github.com/
 * Создать локальный репозиторий
 * Связать удаленный репозиторий с локальным

 Связывание происходит командой `git remote add название ссылка ` в таком формате 
 ```
 git remote add origin https://github.com/Lookiman72/Seminar3.git
 ```

Что бы *вытягивать* информацию , нужна команда `git pull название репозитория ветка` 
в таком виде 
```
 git pull origin main
 ```

 Для разрешения слияния несвязанных историй нужно использовать команду `git pull --allow-unrelated histories название ветка`
выглядить должно так 
```
git pull --allow-unrelated histories origin main
```
После внесения изменений на локальном репозитории , если необходимо их загрузить на удаленный репозиторий , с которым уже связан локальный, нужно использовать команду 
`git push --set-upstream название ветка` 
в таком виде 
``` 
git push --set-upstream origin master
```
Все команды необходимые будут находиться на сайте , которые можно копировать и вставить в терминал
```
В своем аакаунте на GitHub создать копию из другого аккаунта с помощью кнопки "Fork"
```
